...
> **2026-09-26：已完成同盘严格对照。** 历史 GDS A1+A2 结果与公开 Tutti v0.1.1 在同一物理 SSD、同一数据和同一 trace 上对齐；公开 Tutti 的 2,544 条记录全部通过 GPU 数据校验。GDS 无法在当前 DMA mapping 故障下重跑，因此表格明确标注历史/当前的采集日期。详见[关键对照](results/public-v011-cb-20260926/KEY_COMPARISON.md)。
...
以下为同一物理 SSD、同一数据和同一 trace 的参考 miss rate **13.4%** 对照；GDS 是 **2026-09-07** 历史 A1+A2 样本，公开 Tutti v0.1.1 是 **2026-09-26** 新测量：
...
每个配置的公开 Tutti 结果有 200 个正式样本，全部 2,544 条记录均通过数据校验。完整 12 配置表、原始 CSV、输入哈希和时间限制见[关键对照目录](results/public-v011-cb-20260926/)。
